...
{ sets := {t | s ⊆ t},
  univ_sets := subset_univ _,
  sets_of_superset := λ x y hx hsub, subset.trans hx hsub,
  inter_sets := λ x y hx hy, subset_inter hx hy }

def at_top : filter ℕ :=
{ sets := {s | ∃ a, ∀ b, a ≤ b → b ∈ s},
  univ_sets := ⟨37, λ _ _, mem_univ _⟩, 
  sets_of_superset := λ x y hx hsub,
    let ⟨n, hn⟩ := hx in ⟨n, λ b hb, hsub $ hn b hb⟩,
...
{ sets := {s | ∃ ε > 0, Ioo (x - ε) (x + ε) ⊆ s},
  univ_sets := ⟨37, by norm_num, subset_univ _⟩,
  sets_of_superset := λ x y hx hsub,
    let ⟨ε, hε₀, hε₁⟩ := hx in ⟨ε, hε₀, subset.trans hε₁ hsub⟩,
  inter_sets := λ x y hx hy,
    let ⟨ε₀, hε₀, hε₀'⟩ := hx in
    let ⟨ε₁, hε₁, hε₁'⟩ := hy in
      ⟨min ε₀ ε₁, lt_min hε₀ hε₁, subset_inter 
        (subset.trans (Ioo_subset_Ioo (sub_le_sub (le_refl _) $ min_le_left _ _) 
          (add_le_add (le_refl _) $ min_le_left _ _)) hε₀') 
        (subset.trans (Ioo_subset_Ioo (sub_le_sub (le_refl _) $ min_le_right _ _) 
          (add_le_add (le_refl _) $ min_le_right _ _)) hε₁')⟩ }
...
begin
  show (comap m f).ne_bot ↔ _,
  rw ←forall_sets_nonempty_iff_ne_bot,
  split,
    { exact λ h s hs, h (m ⁻¹' s) ⟨s, hs, subset.refl _⟩ },
    { intros h s hs,
      rcases hs with ⟨a, ha₀, ha₁⟩,
      cases h a ha₀ with x hx,
      refine ⟨x, ha₁ $ mem_preimage.2 hx⟩ }
end
...
⟨λ h, ⟨sets_of_superset f h (λ x hx, hx.1), sets_of_superset f h (λ x hx, hx.2)⟩, 
  λ h, inter_sets f h.1 h.2⟩
...
begin
  split; intro h,
    { show (comap coe (𝓝 x)).ne_bot, rw comap_ne_bot_iff,
      intros t ht,
      rcases mem_nhds_sets_iff.1 ht with ⟨s, hs₀, hs₁⟩,
      cases nonempty_inter_iff_exists_right.1 ((mem_closure_iff.1 h) s hs₁.1 hs₁.2) 
        with a ha,
      refine ⟨a, hs₀ ha⟩ },
    { change (comap coe (𝓝 x)).ne_bot at h, rw comap_ne_bot_iff at h,
      rw mem_closure_iff,
      intros o ho hxo,
      rw nonempty_inter_iff_exists_right,
      apply h,
      rw mem_nhds_sets_iff,
      refine ⟨o, subset.refl _, ho, hxo⟩ }
end
...
`A`, it's enough to ask it for `n` large enough, ie. `∀ᶠ n in at_top, u n ∈ A`. ({ n | u n ∈ A } ∈ at_top)
